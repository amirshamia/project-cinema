...
if(j===2||j===7||i===3)cell.isSeat=false
            if (cell.isSeat) {
                cell.price = 5 + i
                cell.isBooked = false
            }
...
function highlightAvailableSeatsAround(board, rowIdx, colIdx){
  ...
  timeOutId= setTimeout(removeHighlight,2500);
  console.log(timeOutId);
}

function removeHighlight(){
    var elMark=document.querySelectorAll('.highlight')
    for (let i = 0; i < elMark.length; i++) {
        const element = elMark[i];
